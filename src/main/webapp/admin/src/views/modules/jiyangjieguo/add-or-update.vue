<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="领养日期" prop="lingyangriqi">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.lingyangriqi" 
                type="datetime"
                placeholder="领养日期">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.lingyangriqi" label="领养日期" prop="lingyangriqi">
              <el-input v-model="ruleForm.lingyangriqi" 
                placeholder="领养日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="寄养结果" prop="jiyangjieguo">
          <el-select v-model="ruleForm.jiyangjieguo" placeholder="请选择寄养结果">
            <el-option
                v-for="(item,index) in jiyangjieguoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="寄养结果" prop="jiyangjieguo">
	      <el-input v-model="ruleForm.jiyangjieguo"
                placeholder="寄养结果" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="领养人姓名" prop="lingyangrenxingming">
          <el-input v-model="ruleForm.lingyangrenxingming" 
              placeholder="领养人姓名" clearable  :readonly="ro.lingyangrenxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="领养人姓名" prop="lingyangrenxingming">
              <el-input v-model="ruleForm.lingyangrenxingming" 
                placeholder="领养人姓名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.lingyangriqi = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.chongwumingcheng = true;
          ...
          if(o=='lingyangriqi'){
            this.ruleForm.lingyangriqi = obj[o];
	    this.ro.lingyangriqi = true;
            continue;
          }
          if(o=='jiyangjieguo'){
            this.ruleForm.jiyangjieguo = obj[o];
	    this.ro.jiyangjieguo = true;
            continue;
          }
          if(o=='lingyangrenxingming'){
            this.ruleForm.lingyangrenxingming = obj[o];
	    this.ro.lingyangrenxingming = true;
            continue;
          }
          if(o=='lianxifangshi'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.jiyangjieguoOptions = "成功,失败".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `jiyangjieguo/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiyangjieguo/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiyangjieguoCrossAddOrUpdateFlag = false;
                  this.parent.search();
