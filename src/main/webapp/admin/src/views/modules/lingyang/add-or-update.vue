      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="宠物编号" prop="chongwubianhao">
          <el-input v-model="ruleForm.chongwubianhao" 
              placeholder="宠物编号" clearable  :readonly="ro.chongwubianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="宠物编号" prop="chongwubianhao">
              <el-input v-model="ruleForm.chongwubianhao" 
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `lingyang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.lingyangCrossAddOrUpdateFlag = false;
                  this.parent.search();
